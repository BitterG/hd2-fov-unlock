...
4. **分帧推进，不占主线程**：`.text` 扫描按 256KB 块 + `os.clock()` 预算（每步 ≤1.5ms）
   跨帧做完；身份校验同样有预算且可续跑；通道①（代码扫描）超时就放弃并说明。
   启动期不会再有哪一帧吃掉几十 MB 的读取 —— 之前那种"有时候启动卡住 / 窗口未响应、
   多试几次才好"就是它造成的。同时 mod 内部**不再调用 `os.execute`**：那会从游戏进程里
   spawn `cmd.exe`，在 GameGuard 环境下是典型的高风险动作。
